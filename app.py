...
def convert_df_to_excel(df_to_export):
    keep_cols = [c for c in df_to_export.columns if any(k in c for k in ['الاسم', 'الهاتف', 'رقم', 'العدد', 'أفراد', 'الإقامة', 'فندق', 'انطلاق', 'مكان', 'تكلفة', 'مبلغ'])]
    if not keep_cols:
...
if st.sidebar.button("🔄 سحب وتحديث البيانات الشاملة", use_container_width=True):
    ...
    except Exception as e: st.sidebar.error(f"خطأ: {e}")
...
if page == "💬 مركز مراسلة حالات الزبائن":
    ...
    if 'df' in st.session_state:
        ...
        col_count = next((c for c in df.columns if 'العدد' in c or 'أفراد' in c), None)
        ...
        if col_name and col_phone:
            ...
            phone_str = str(user_data[col_phone]).replace('.0','')
            
            msg_confirm = f"السلام عليكم،\nمرحباً بك في *شركة قصر الهناء* 🌹\nتم استلام بياناتكم بنجاح 📊✨\n\n📌 *الاسم:* {user_data[col_name]}\n👥 *العدد:* {user_data.get(col_count, 'غير محدد')}\n🏨 *الإقامة:* {user_data.get(col_hotel, 'غير محدد')}\n\n*شكراً لثقتكم!* 🏔️"
            url_confirm = get_wa_intent(phone_str, msg_confirm)
            
            st.markdown(f'<a href="{url_confirm}" target="_blank"><button style="background-color: #2b5c8f; color: white; padding: 12px; border-radius: 6px; width: 100%;">🔵 1. إرسال تأكيد الاستلام</button></a>', unsafe_allow_html=True)
    ...
    if 'df' in st.session_state:
        ...
        if col_name:
            ...
            if search_user != "-- اختر اسماً --":
                ...
                col_cost = next((c for c in df.columns if 'تكلفة' in c or 'مبلغ' in c or 'سعر' in c), 'غير مسجل')
                cost_val = user_full_data.get(col_cost, 'غير محدد')
                st.markdown(f"""
                <div style="background-color: #f8f9fa; border-right: 5px solid #25D366; padding: 20px; border-radius: 8px;">
                    <h3>🎫 بطاقة البيانات التفصيلية</h3>
                    <p><b>👤 الاسم:</b> {user_full_data.get(col_name, 'غير مسجل')}</p>
                    <p><b>💰 إجمالي التكلفة:</b> <span style="color:red; font-size:20px;">{cost_val}</span></p>
                </div>
                """, unsafe_allow_html=True)

# ----------------------------------------------------
# باقي الصفحات (نفس منطقك الأصلي للـ Excel والـ Dataframe)
# ----------------------------------------------------
elif "كشف" in page or "الكشف" in page:
    if 'df' in st.session_state: display_styled_dataframe(st.session_state['df'])
