...
def main_system():
    ...
    # 🔍 الصفحة الثانية: استعلام وبطاقة حجز عميل
        ...
        if 'df' in st.session_state:
            ...
            # البحث الدقيق عن عمود "اجمالي التكلفة" أو أي عمود مالي شبيه
            col_money = next((c for c in df.columns if any(k in c for k in ['اجمالي التكلفة', 'إجمالي التكلفة', 'مبلغ', 'قيمة', 'مدفوع'])), None)
            ...
            if col_name:
                ...
                if search_user != "-- اختر اسماً لعرض تفاصيل حركته --":
                    ...
                    money_value = user_full_data.get(col_money, 'غير محدد أو لم يدرج بعد') if col_money else "لم يتم العثور على عمود التكلفة المالي"
                    ...
                    <div style="background-color: #f8f9fa; border-right: 5px solid #1d3557; padding: 20px; border-radius: 8px; box-shadow: 2px 2px 5px rgba(0,0,0,0.05);">
                        ...
                        <div style="background-color: #e3f2fd; padding: 12px; border-radius: 6px; border-right: 4px solid #1565c0; margin-top: 15px;">
                            <p style="font-size: 17px; margin: 0; color: #0d47a1;"><b>💰 إجمالي التكلفة المالية للعميل:</b> <span style="font-weight: bold; font-size: 18px;">{money_value}</span></p>
                        </div>
                    </div>
